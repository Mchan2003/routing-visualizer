...
prims_algo(shared_ptr<Node> &src_ptr) {
    ...
    vector<tuple<int, shared_ptr<Node>, shared_ptr<Node>>> adj_vector;
    ...
    // push adjacent nodes from starting node into queue
    ...
    while (!adj_queue.empty()) {
        ...
        cout << "source: " << get<1>(min_edge)->label << " "
             << "destination: " << get<2>(min_edge)->label << " "
             << "weight: " << get<0>(min_edge) << " \n";

        // check to see if label for adjacent node is already in seen vector and
        // move to next loop
        if(seen.find(get<1>(min_edge)->label) != seen.end() && seen.find(get<2>(min_edge)->label) != seen.end())
        {
            continue;
        }
        ...
        // mark the destination node as visited by adding char label to seen
        // vector
