...
void Node::remove_edge(char dest) {
    ...
    dest_ptr->adj.erase(
            std::find_if(dest_ptr->adj.begin(), dest_ptr->adj.end(),
                         [&](const auto &p) { return p.first == dest_ptr; }));

    if (dest_ptr.use_count() == 1) {
        all_nodes.erase(
                std::find_if(all_nodes.begin(), all_nodes.end(),
                             [&](const auto &p) { return p.second == dest_ptr; }));
    }
}